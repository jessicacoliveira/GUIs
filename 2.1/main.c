...
int main (int argc, char* args[])
{
    ...
    assert(img_background != NULL);
    

    /* EXECUÇÃO */

	int running=1, counter_click=0, aux_special=0, aux_jump = 1;
	ClickInfo click = {0, 0, 0};
	int x, y;
		...
		if(isevt){
			switch(state_){
				case STATE_READY:
					if(evt.type == SDL_MOUSEBUTTONDOWN){
						...
						click.counter++;
						click.x0 = evt.button.x;
						click.y0 = evt.button.y;
					}
					break;
				case STATE_CLICKING:
					if(evt.type == SDL_MOUSEBUTTONDOWN){ 
						if (AUX_ClickTimeoutCount(&last_click_time, TIMEOUT_CLICK)){
							...
							if (click.counter>0){
								...
								multiclick.user.code = click.counter;
						...
						dif_x = click.x0 - x;
						dif_y = click.y0 - y;
						// considera um limite de 5 pixels de tolerancia (desnecessario, mas feito)
						if (!((dif_x >= -LIMIT_PIXELS && dif_x <= LIMIT_PIXELS) &&
							(dif_y >= -LIMIT_PIXELS && dif_y <= LIMIT_PIXELS))) {
								if (click.counter > 0) {
									SDL_Event multiclick;
									multiclick.type = SDL_USEREVENT;
									multiclick.user.code = click.counter;
            ...
            switch (animamode_){
                ...
                case MODE_WALKING:
					...
					if (player_center < click.x0) {
						player.x += 10;
						if (player.x + player.w/2 > click.x0) player.x = click.x0 - player.w/2;
						if (player.x + player.w/2 == click.x0) animamode_ = MODE_STANDING;
					}
					else if (player_center > click.x0) {
					    player.x -= 10;
					    if (player.x + player.w/2 < click.x0) player.x = click.x0 - player.w/2;
					    if (player.x + player.w/2 == click.x0) animamode_ = MODE_STANDING;
					}           
